module.exports =  {
    ...
    async execute(message, db){
        message.channel.send(':construction: This command is still in construction :construction:')
        ...
        if(player_number >= 15){
            while(a !== 15){
                content  = content + `**${a+1}.** ${data[a].data.nome} **- ${data[a].data.total}**\n`;
                a++;
            }
        }else{
        ...
        content = content + `A tua posição atual é ${message_user+1}º lugar com ${data[message_user].data.total} milisegundos.`
